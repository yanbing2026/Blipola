...
export function createBlipola({ progress }) {
  return {
    recommend(skill, item) {
      ...
      return { action, difficulty: getDifficulty(progress, skill, item), reason: getRecommendationReason(progress, skill, item), message: say(action === "NEW" ? "newSkill" : action === "CHALLENGE" ? "challenge" : "practice") };
    },
